...
class KikKatCardReader(activity: Context) : CardReader(activity) {
    ...
    override fun enableCardReader() {
        ...
        if (extra != null && extra!!.getInt(NfcAdapter.EXTRA_READER_PRESENCE_CHECK_DELAY) > 0) {
            mDefaultAdapter?.enableReaderMode(mActivity as Activity?, mReaderCallback, READER_FLAG, extra)
        } else {
            mDefaultAdapter?.enableReaderMode(mActivity as Activity?, mReaderCallback, READER_FLAG, null)
        }
    }

    override fun disableCardReader() {
        super.disableCardReader()
        mDefaultAdapter?.disableReaderMode(mActivity as Activity?)
    }
