...
internal object Util {
    ...
    /**
     * 是否开启NFC
     * @suppress OPPO A37m 发现必须同时开启NFC以及Android Beam才可以使用
     */
    fun isEnableNfc(context: Context): Boolean {
        val nfcAdapter = NfcAdapter.getDefaultAdapter(context)
        if (Build.MANUFACTURER.toUpperCase().contains("OPPO")) {
            return nfcAdapter.isEnabled && isSupportBeam(context)
        }
    ...
    fun isSupportBeam(context: Context): Boolean {
        val nfcAdapter = NfcAdapter.getDefaultAdapter(context)
     ...
     */
    fun intentToNfcSetting(context: Context) {
