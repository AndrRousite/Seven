 ...
 */
class TextDrawable(val builder: Builder) : ShapeDrawable() {
    private var textPaint: Paint
    private var borderPaint: Paint
    private val shadeFactor = 0.9f
    ...
    init {
        ...
        // text and color
        text =
            if (builder.toUpperCase) builder.text?.substring(0, 1)?.toUpperCase() ?: "#" else builder.text?.substring(
                0,
                1
            ) ?: "#"
        color = builder.color
    ...
    private fun getDarkerShade(color: Int): Int {
        return Color.rgb(
            (shadeFactor * Color.red(color)).toInt(),
            (shadeFactor * Color.green(color)).toInt(),
            (shadeFactor * Color.blue(color)).toInt()
        )
    }
