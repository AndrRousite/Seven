...
import com.weyee.sdk.print.manager.ble.leBle.BluetoothLeListener
...
class BluetoothActivity : BaseActivity<BasePresenter<BaseModel, IView>>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        adapter = object : BaseAdapter<Any>(null, { _, _, data, _ ->
            if (data is BluetoothDevice && data.bondState != BluetoothDevice.BOND_BONDED) {
                bluetoothUtils.connect(data.address)
            }
        }) {
                    ...
                    override fun setData(data: Any, position: Int) {
                        if (data is BluetoothDevice) {
                            ...
                            if ("未配对设备(点击名称进行配对)" == data) {
                                ...
                                qmuiButton.setBorderColor(resources.getColor(if (bluetoothUtils.isScanning) R.color.cl_666666 else R.color.cl_50a7ff))
                                qmuiButton.setTextColor(resources.getColor(if (bluetoothUtils.isScanning) R.color.cl_666666 else R.color.cl_50a7ff))
                            } else {
    ...
    override fun initData(savedInstanceState: Bundle?) {
        bluetoothUtils = BluetoothUtils(DeviceCode.getDeviceCode(), object : BluetoothLeListener {
            ...
            override fun onConnectionStateChange(device: BluetoothDevice?, status: Int, newState: Int) {
                when (newState) {
                    ...
                    IBluetoothAble.STATE_WRITE_COMPLETED -> {
                        hideProgress()
                        ToastUtils.show("打印完成")
                    }
                    IBluetoothAble.STATE_READ_COMPLETED -> {
                        // 读取到数据
                    }
                    BluetoothDevice.BOND_BONDED -> {
                        ...
                        list.addAll(1, bluetoothUtils.bondedDevices.toList())
